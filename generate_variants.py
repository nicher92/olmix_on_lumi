...
def parse_yaml(datasets_config):
    ...
    # 2. Parse YAML, count tokens (appending .bin), and build SourceConfigs
...
def calculate_priors_and_variants(leaf_tokens):
    # 3. Calculate priors and variant counts
...
# 5. Save the raw JSON (Optional, but good for record keeping)
lumi_variants = []
...
# 6. Generate Megatron .txt mix files and the bash launcher
...
# 7. Create master launcher script
...
print(f"✅ Generated {launcher_script}. Run it with: ./launch_all_swarms.sh")
